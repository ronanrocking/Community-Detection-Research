"""Create an ACM t-SNE using the training logic in 3#dynamicAlgo/main.py."""
...
def plot_tsne(
    ...
    color_norm = plt.Normalize(vmin=label_values.min(), vmax=label_values.max())
    legend_handles = [
        Line2D(
            [0],
            [0],
            marker="o",
            linestyle="",
            color=color_map(color_norm(value)),
            label=str(int(value)),
            markersize=6,
        )
...
def main() -> None:
    args = Namespace(
        ...
        consensus_runs=15,
        perplexity=30.0,
        output=Path("visualizations") / "acm_dynamic_tsne.png",
    )
    data = load_data(
        "./", "acm", "tensor", "npy", "npy", False, False, False
    )
